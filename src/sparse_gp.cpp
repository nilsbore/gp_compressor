...
sparse_gp::sparse_gp(int capacity, float s20, float sigmaf, float l) :
    capacity(capacity), s20(s20), sigmaf_sq(sigmaf*sigmaf), l_sq(l*l), eps_tol(1e-6f)
{

}

//Add a chunk of data
void sparse_gp::add_measurements(const MatrixXf& X,const VectorXf& y)
{
    for (int i = 0; i < X.cols(); i++) {
        add(X.row(i).transpose(), y(i)); // row transpose
    }
}

//Add this input and output to the GP
void sparse_gp::add(const VectorXf& X, float y)
{
    total_count++;
    if (DEBUG) {
    ...
    if (current_size == 0){//First point is easy
        ...
        VectorXf e_hat = Q*k;//Appendix G, section c
        //residual length
        ...
        if (gamma < eps_tol && capacity != -1) {//Nearly singular, do a sparse update (e_tol)
            if (DEBUG) {
                printf("Sparse %lf \n",gamma);
            }
            double eta = 1/(1 + gamma*r);//Ibid
            VectorXf s_hat = C*k + e_hat;//Appendix G section e
            alpha += s_hat*(q * eta);//Appendix G section f
            C = C + r*eta*s_hat*s_hat.transpose();//Ibid
        }
        else { //Full update
            if (DEBUG) {
                printf("Full!\n");
            ...
            //Update gram matrix
            Q = Q + (1/gamma)*e_hat*e_hat.transpose();//Equation 3.5
...
void sparse_gp::delete_bv(int loc)
{
    ...
    float alphastar = alpha(loc);
    alpha(loc) = alpha(alpha.rows() - 1);
    alpha.conservativeResize(alpha.rows() - 1);

    //Now C
    double cstar = C(loc, loc);
    VectorXf Cstar = C.col(loc);
    Cstar(loc) = Cstar(Cstar.rows() - 1);
    Cstar.conservativeResize(Cstar.cols() - 1);

    VectorXf Crep = C.col(C.cols() - 1);
    ...
    //VectorXf qc = (Qstar + Cstar)/(qstar + cstar);
...
void sparse_gp::predict_measurements(VectorXf& f_star, const MatrixXf& X_star, VectorXf& sigconf, bool conf)
{
...
float sparse_gp::predict(const VectorXf& X_star, float& sigma, bool conf)
{
...
double sparse_gp::log_prob(const VectorXf& X_star, const VectorXf& f_star)
{
    int dout = f_star.rows();
    static const double logsqrt2pi = 0.5f*log(2.0f*M_PI);
    double sigma;
    MatrixXf Sigma(dout, dout);
    ...
    long double lp = -float(dout)*logsqrt2pi - 0.5f*log_det_cov - 0.5f*cent2/sigma;
...
void sparse_gp::construct_covariance(VectorXf& K, const Vector2f& X, const MatrixXf& Xv)
{
    K.resize(Xv.rows());
...
float sparse_gp::kernel_function(const Vector2f& xi, const Vector2f& xj)
{
